...
__global__
void silly_attn_mult(float *out, float* out_l, float *K, float *Q, float* V, float scaling, int batch_stride, int T_r, int T_c)
{
  ...
  */
  ...
  // load Q_i
  for (int t=0; t<num_tiles; t++){
    Q_i[tid_y][t * B_c + tid_x] = Q[batch_offset + (blockIdx.y * B_r + tid_y) * d + tid_x];
  ...
  for (int j = 0; j < T_c; j++) {
    ...
    out[batch_offset + tid_y * d + tid_x + B_c * t] = S_i[tid_y][tid_x];
